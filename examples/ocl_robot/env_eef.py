...
from examples.ocl_robot.umi_replay import ros_oper_eef as _ros_oper
...
ros_config = {
    ...
    "puppet_arm_left_cmd_topic_eef": "/pos_cmd_left",
    "puppet_arm_right_cmd_topic_eef": "/pos_cmd_right",

    "puppet_arm_left_topic_eef": "/puppet/end_pose_left",
    "puppet_arm_right_topic_eef": "/puppet/end_pose_right",
    "puppet_arm_left_topic": "/puppet/joint_left",
    "puppet_arm_right_topic": "/puppet/joint_right",
...
class PiperRealEnv:
    ...
    Action space:      [left_arm_eef (7),             # end position
                        left_gripper_positions (1),    # normalized gripper position (0: close, 1: open)
                        right_arm_eef (7),            # end position
                        right_gripper_positions (1),]  # normalized gripper position (0: close, 1: open)

    Observation space: {"qpos": Concat[ left_arm_eef (7),          # end position
    ...
    def __init__(self, init_node, *, reset_pos:Optional[List[float]] = None, setup_robots: bool = False):
        if init_node:
            rospy.init_node('eef_state_publisher_debug', anonymous=True)
    ...
    def reset(self,*, fake=False):
        if not fake:
            ...
            # )
            self.pre_action = origin_obs["qpos"].copy()
        return dm_env.TimeStep(
            step_type=dm_env.StepType.FIRST,
            reward=self.get_reward(),
            discount=None,
            observation=origin_obs
        )
    ...
    def get_observation(self):
        ...
        while True and not rospy.is_shutdown():
            ...
            qpos = np.concatenate([np.array([puppet_arm_left.pose.position.x, puppet_arm_left.pose.position.y, puppet_arm_left.pose.position.z, ]), np.array([puppet_arm_left.pose.orientation.x, puppet_arm_left.pose.orientation.y,puppet_arm_left.pose.orientation.z,puppet_arm_left.pose.orientation.w,]),np.array(master_arm_left.position[-1:]),
                np.array([puppet_arm_right.pose.position.x, puppet_arm_right.pose.position.y, puppet_arm_right.pose.position.z]), np.array([puppet_arm_right.pose.orientation.x,puppet_arm_right.pose.orientation.y,puppet_arm_right.pose.orientation.z,puppet_arm_right.pose.orientation.w,] ),np.array(master_arm_right.position[-1:])], axis=0)
            obs = {
                    'qpos': qpos,
                    'images':
                        {
    ...
    def step(self, action, STOP=False):
        ...
        for act in interp_actions:
            ...
            print(f"Publishing puppet arm actions: left_action={left_action}, right_action={right_action}")
            self.ros_operator.puppet_arm_publish_eef(left_action, right_action)  # puppet_arm_publish_continuous_thread

            if ros_config["use_robot_base"]:
                vel_action = act[16:16]
