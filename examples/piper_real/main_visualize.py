...
class _RealtimeVisualizeSubscriber(_subscriber.Subscriber):
    def __init__(self, environment, visualizer, *, prompt: str) -> None:
        self._environment = environment
        self._visualizer = visualizer
        self._prompt = prompt

    def on_episode_start(self) -> None:
        self._visualizer.set_subtask_context(1, 1, "policy", self._prompt)
    ...
    def on_step(self, observation: dict, action: dict) -> None:
        del observation
        step_idx = max(int(self._environment.get_cursor()) - 1, 0)
        progress = action.get("progress")
        extra_info = f"policy prompt: {self._prompt}"
        if progress is not None:
            extra_info = f"{extra_info} | progress: {float(progress):.4f}"
        self._visualizer.update(step_idx, extra_info=extra_info)
...
def _create_realtime_visualizer(args: Args, environment):
    if not args.visualize and not args.save_path:
        return None
    from examples.piper_real import replay_visualizer as _replay_visualizer

    return _replay_visualizer.ReplayVisualizer(
        environment,
...
def _run_real_hybrid(args: Args, prompt: str) -> None:
    ...
    visualizer = (
        _create_realtime_visualizer(args, environment)
        if environment is not None
        else None
    )
    if args.visualize and visualizer is None:
        logging.warning(
            "--visualize requested, but no ROS camera environment is active for this run."
        )
...
def main(args: Args) -> None:
    ...
    if not args.use_llm_planner or not prompt:
        ...
        visualizer = _create_realtime_visualizer(args, environment)
        if visualizer is not None:
            visualizer.set_subtask_context(1, 1, "policy", args.prompt)
            subscribers.append(
                _RealtimeVisualizeSubscriber(
                    environment,
                    visualizer,
                    prompt=args.prompt,
                )
